...
function ivrss() 
{
	...
	$sSql = "select * from ".WP_ivrss_TABLE." where ivrss_status = 'YES' and ivrss_type = %s ";
	
	$sSql = $wpdb->prepare($sSql, $ivrss_type);
	...
	if ( ! empty($data) ) 
	{
		...
		$ivrss_scrollerheight = $ivrss_scrollerheight;
...
function ivrss_shortcode( $atts ) 
{
	...
	$ivrss_type = isset($atts['type']) ? $atts['type'] : '';	
	$ivrss_scrollercount = isset($atts['display']) ? $atts['display'] : '';	
	$ivrss_scrollerheight = isset($atts['height']) ? $atts['height'] : '';	
	$ivrss_random = isset($atts['random']) ? $atts['random'] : '';	
...
function ivrss_adminscripts() 
{
	if( !empty( $_GET['page'] ) ) 
	{
		switch ( $_GET['page'] ) 
		{
			case 'image-vertical-reel-scroll-slideshow':
				wp_register_script( 'ivrss-adminscripts', plugins_url( 'pages/setting.js', __FILE__ ), '', '', true );
				wp_enqueue_script( 'ivrss-adminscripts' );
				$ivrss_select_params = array(
					'ivrss_path'   	=> __( 'Please enter the image path.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_link'   	=> __( 'Please enter the target link.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_target' 	=> __( 'Please select the target option.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_title' 	=> __( 'Please enter the image alt text.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_order'  	=> __( 'Please enter the display order, only number.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_status' 	=> __( 'Please select the display status.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_type'  	=> __( 'Please select the gallery type.', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
					'ivrss_delete'	=> __( 'Do you want to delete this record?', 'ivrss-select', 'image-vertical-reel-scroll-slideshow' ),
				);
				wp_localize_script( 'ivrss-adminscripts', 'ivrss_adminscripts', $ivrss_select_params );
				break;
		}
	}
}
...
add_action( 'admin_enqueue_scripts', 'ivrss_adminscripts' );
?>